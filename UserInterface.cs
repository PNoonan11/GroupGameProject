...
public void firstChallenge()
{
    ...
    else if (userAnswerOne != answerOne)
    {
        Console.Clear();
    ...
    else if (userAnswerOne == "1")
    {
        ...
        if (useItem = "4")
        {
            Console.WriteLine("Which item would you like to use? Type 1, 2, or 3... or n to exit");
            string useWhichItem = Console.ReadLine();
            if (useWhichItem = "1" && _repo.trevorHelp == 1)
            {
                Console.WriteLine("The answer to our first riddle is a-(-b);");
                _repo.trevorHelp --;
            }
            else if (useItemWhichItem = "1" && _repo.trevorHelp == 0)
            {
